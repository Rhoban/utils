...
double PolySpline::get(double x) { return interpolation(x, Value); }

double PolySpline::getVel(double x) { return interpolation(x, Speed); }

/**
 * Access to internal Points container
 */
const PolySpline::Points &PolySpline::points() const { return _points; }
...
PolySpline::Polynom PolySpline::polynomFit(double t1, double val1,
                                           double delta1, double t2,
...
void PolySpline::computeSplines() {
  ...
  for (size_t i = 1; i < _points.size(); i++) {
    ...
    struct Spline spline = {
        polynomFit(_points[i - 1].position, _points[i - 1].value,
                   _points[i - 1].delta, _points[i].position, _points[i].value,
                   _points[i].delta),
